...
from modules.database import load_projects_data, load_users_data
...
def to_xlsx(name: str, diapason: tuple, df: pd.DataFrame, unit_type):
    ...
    for index, row in output_df.iterrows():
        ...
        sheet[f'I{12 + index - 1}'] = row['ИТОГО']
        sheet.insert_rows(12 + index)
    sheet[f'I{12 + last_row + 1}'] = f'=SUM(I12:I{12 + last_row})'
    sheet[f'L{12 + last_row + 1}'] = f'=SUM(L12:L{12 + last_row})'
    exel_styling(sheet, last_row)
    book.save(binary_output)
    return binary_output


def to_xlsx_group(name: str, diapason: tuple, df: pd.DataFrame, unit_type):
    match diapason:
        case date_start, date_end:
            date_end = date_end
        case (date_start,):
    ...
    output_df.drop(len(output_df), inplace=True, axis=0)
    last_row = len(output_df)
    book = openpyxl.load_workbook(r"Z:\11. Bim-отдел\data\бланк_отдел.xlsx")
    sheet = book.active
...
def accountant_report_xlsx(df: pd.DataFrame):
    binary_output = BytesIO()
    df.to_excel(binary_output, index=False, sheet_name='TDSheet', startcol=1)
    return binary_output
...
class ReportFormHead:
    def __init__(self, user_class):
        ...
        self.resources = load_users_data()
        self.job_types = job_types()
        match self.user.group_code:
            case group_code_1, group_code_2:
                self.user_jobs = self.job_types[
                    (self.job_types['group_code'] == str(group_code_1)) | (self.job_types['group_code'] == str(group_code_2))
                ]
            case group_code:
        ...
        match self.user.level:
            case Levels.admin:
                self.resource_name = st.selectbox(
                    'Ф.И.О',
                    options=self.resources['name'].unique(),
...
class ReportFormBody:
    def __init__(self, user_class, field_number, user_jobs):
        self.data_dataframe = None
        self.projects = load_projects_data()
        self.user_group_code = user_class.group_code
    ...
    def project_selector(self, field_index):
        match self.user_group_code:
            case '3' | '5' | '8':
                options = self.projects[self.projects['type'] != 'ЭП']['name'].sort_values()
            case _:
                options = self.projects['name'].sort_values()
        self.project_name = st.selectbox(
            'Проект:',
            options=options,
            key=f'Проект_{field_index}'
        )
        sections_string = self.projects[self.projects['name'] == self.project_name]['sections'].iloc[0]
        self.sections = sections_string.split(',')

    def section_selector(self, field_index):
        self.section = st.multiselect(
            'Блок:',
            key=f'Блок_{field_index}',
            options=self.sections,
            default=self.sections[0]
        )
    ...
    def get_project_id(self):
        project_id = self.projects[self.projects['name'] == self.project_name]['code'].iloc[0]
        return project_id
    ...
    def get_job_by_project(self):
        match self.project_name:
            case '000_Без проекта':
                return self.job
            case _:
                project_type = self.projects[self.projects['name'] == self.project_name]['type'].iloc[0]
    ...
    def form(self, user_jobs):
        ...
        unfilled = [i for i in self.collected_data if i[6] == 0 or i[10] == '' or i[3] == '']
        if not unfilled:
            ...
            st.warning('Заполните **Часы** и **Описание работы** и выберите **Блок**')
